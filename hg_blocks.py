...
def create_hourglass_network(num_classes, num_stacks, num_channels, input_shape, bottleneck):
...
def create_left_half_blocks(bottom, bottleneck, hglayer, num_channels):
    ...
    _x = MaxPooling1D(pool_size=2, strides=2)(f1)

    f2 = bottleneck(_x, num_channels, hgname + '_l2')
    _x = MaxPooling1D(pool_size=2, strides=2)(f2)

    f4 = bottleneck(_x, num_channels, hgname + '_l4')
    _x = MaxPooling1D(pool_size=2, strides=2)(f4)
